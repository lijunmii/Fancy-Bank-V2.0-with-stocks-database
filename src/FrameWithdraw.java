...
public class FrameWithdraw extends JFrame {
    ...
    FrameWithdraw() {}
    FrameWithdraw(FrameATM frameATM, BankDataBase bankDataBase) {
        ...
        JPanel panel_1 = new JPanel(); {
            ...
            JPanel panel_1_1 = new JPanel(); {
                ...
                JLabel title = new JLabel("<html>Withdraw - <font color='#8A2BE2'>FANCY BANK</font>™</html>", JLabel.CENTER);
                panel_1_1.add(title);
            }
            panel_1.add(panel_1_1);
            panel_1.add(new JLabel("  Your accounts are listed below.", JLabel.LEFT));
            panel_1.add(new JLabel("  Select an account you want to withdraw from.", JLabel.LEFT));
        }
        panel.add(panel_1);
        ...
        JPanel panel_3 = new JPanel(); {
            ...
            panel_3.add(new JLabel("Choose a currency, enter amount, then click Withdraw.", JLabel.CENTER));
                ...
                if (amountStr.length() == 0) {
                    ...
                    if (isNumeric(amountStr)) {
                        ...
                        if (bankDataBase.getClient(username).getAccounts().get(accountIndex).able2Transaction(amount)) {
                            bankDataBase.withdraw(username, accountIndex, amount);
                            frameATM.setClient(bankDataBase.getClient(username));
                            JOptionPane.showMessageDialog(this, "Withdraw Success.", "SUCCESS", JOptionPane.INFORMATION_MESSAGE);
                            dispose();
                        } else {
